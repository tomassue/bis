<?php 
	...
	if(!empty($username)){
        ...
        if($new_pass==$con_pass){
            ...
            $check = "SELECT * FROM tbl_users WHERE user_username='$username' AND `password`='$cur_pass'";
            $res = $conn->query($check);

            if($res->num_rows){

                $query 		= "UPDATE tbl_users SET `password`='$new_pass' WHERE user_username='$username'";	
