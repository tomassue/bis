							...
							<div class="card">
								<div class="card-header">
									<div class="card-head-row">
										<div class="card-title">Add Resident [CO-OCCUPANT] <i>(Page 2)</i></div>
									</div>
								</div>
								<div class="card-body">
									<form method="POST" action="model/save_resident2.php" onclick="myFunction()" enctype="multipart/form-data" onsubmit="return confirm('Are you sure you want to submit this form?');">
														...
														<div class="form-group">
															...
															<select class="form-control js-states input-lg " style="width:100%;" id="single" name="householdnumber" required>
																<?php foreach ($household as $row2) : ?>
																	<option value=""></option>
																	<option value="<?= $row2['id_household'] ?>">No. <?= $row2['household_number'] . ' (' . $row2['firstname'] . ' ' . $row2['lastname'] . ')' ?> </option>
																<?php endforeach ?>
															</select>
															<small class="form-text text-muted text-primary">Enter the <u>HOUSE NO. or the NAME OF THE PERSON</u> who's the resident associated with. E.g., the head of the family.</small>
														</div>
										...
										<div class="modal-footer">
											<input type="hidden" name="residence_type" value="co-occupant">
	...
	<script>
		...
		$(function() {
			$("#householdnumber").autocomplete({
				source: 'backend-script_residentCo-occupant.php'
			});
		});
